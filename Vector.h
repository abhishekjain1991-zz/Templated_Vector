...
class Vector
{
 public:
  ...
  // Define the iterator
  // Iterator initializers, const version first, non-const second
  ConstVectorIterator<T> Begin() const; // Get an iterator to first element
  VectorIterator<T>      Begin(); // Get an iterator to first element
  ConstVectorIterator<T> End() const;   // Get an iterator to one beyond end
  VectorIterator<T>      End();   // Get an iterator to one beyond end
#ifdef GRAD_STUDENT
...
// Define the ConstVectorIterator here.....
//
template <typename T>
class ConstVectorIterator 
{
 public:
  ...
  ConstVectorIterator(const ConstVectorIterator&);
  // De-referencing, and incrementing operators
  // Note we are not implementing the "arrow" (->) operator
  // Note we are not implementing the decrementing operators
  T& operator*() const;
  ConstVectorIterator operator++();          // Prefix increment  
  ConstVectorIterator operator++(int);        // Postfix increment
  // Comparison operators
  bool operator !=(const ConstVectorIterator&) const;
  bool operator ==(const ConstVectorIterator&) const;
 private:
  //Member variables
  T* current;
